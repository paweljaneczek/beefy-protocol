...
contract StrategyCommonChefLP is StratManager, FeeManager {
    ...
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _mofiFeeRecipient) {
        want = _want;
        poolId = _poolId;
        chef = _chef;
    ...
    function chargeFees() internal {
        uint256 toNative = IERC20(output).balanceOf(address(this)).mul(HARVEST_FEE).div(MAX_FEE);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toNative, 0, toNativeRoute, address(this), block.timestamp);
    ...
    function chargeFeesNative() internal {
        uint256 nativeFees = IERC20(native).balanceOf(address(this)).mul(HARVEST_FEE).div(MAX_FEE);
    ...
    function transferFees(uint256 amount) internal {
        ...
        uint256 mofiFeeAmount = amount.mul(mofiFee).div(MAX_FEE);
        IERC20(native).safeTransfer(mofiFeeRecipient, mofiFeeAmount);
